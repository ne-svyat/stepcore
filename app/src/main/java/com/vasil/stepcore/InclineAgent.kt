...
object InclineAgent {
    ...
    /** Порог доверия для автометки: нижняя граница Уилсона. Ровно 90%
     *  достигается серией 35/35 на направление. Одна ошибка (34/35 =
     *  85.5%) отбрасывает далеко: вернуться можно лишь на 52/53. */
    const val AUTO_TRUST = 0.90f
    /** Автометку ставим только с ощутимым запасом до границы: у самой
     *  черты пусть решает человек. */
    const val AUTO_MARGIN = 0.30f
    /** Контрольная доля: каждая N-я подходящая сессия всё равно идёт в
     *  вопросы, иначе перестанем замечать дрейф походки. */
    const val AUTO_CONTROL_EVERY = 5

    /** Заслужило ли направление право на автометку. */
    fun trusted(k: Int, n: Int): Boolean = wilsonLower(k, n) >= AUTO_TRUST

    /** Сколько СЛЕПЫХ проверок нужно, прежде чем счёту можно верить.
     *  Слепая проверка - вопрос без подсказки: агент предсказал молча,
     *  человек ответил не зная вердикта. Только это измеряет правоту,
     *  а не согласие под наводящим вопросом. */
    const val BLIND_MIN = 12
    /** Каждый N-й вопрос задаётся вслепую. Реже - долго набирать,
     *  чаще - человеку тяжелее (без подсказки думать дольше). */
    const val BLIND_EVERY = 3

    /** Право на автометку: только по слепым проверкам и только когда их
     *  набралось достаточно. Подсказанный счёт прав не даёт. */
    fun trustedBlind(k: Int, n: Int): Boolean =
        n >= BLIND_MIN && wilsonLower(k, n) >= AUTO_TRUST
