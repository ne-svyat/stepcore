...
class Corpus(raw: String) {
    ...
    fun renderOrNull(
        ...
        // Анти-повтор: стартовая точка выбирается броском, но каждое следующее
        // срабатывание ЭТОГО ключа сдвигается на вариант вперёд. Иначе кубик
        // честно, но скучно выдаёт одну и ту же метель пять раз за экспедицию.
        val base = (((roll % pool.size) + pool.size) % pool.size).toInt()
        val idx = (base + (nth % pool.size)) % pool.size
        var t = pool[idx].text
