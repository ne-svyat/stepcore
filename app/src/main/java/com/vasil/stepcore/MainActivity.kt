...
class MainActivity : AppCompatActivity() {
    ...
    private suspend fun refreshTodayEnergy() {
        ...
        // v291. У дистанции и активного времени объяснения не было вовсе:
        // рамка рисовалась, а нажатие никуда не вело. Человек справедливо
        // ждал того же окна, что у калорий - соседние плашки выглядят
        // одинаково, значит и вести себя должны одинаково.
        findViewById<View>(R.id.distanceChip).setOnClickListener { showDistanceInfo() }
        findViewById<View>(R.id.activeTimeChip).setOnClickListener { showActiveTimeInfo() }
    }

    /** Откуда берётся дистанция. Честно про то, что её никто не измеряет. */
    private fun showDistanceInfo() {
        // Показываем ИЗМЕРЕННУЮ длину шага, а не расчётную на лету:
        // расчётная зависит от каденса часа, и в диалоге это была бы
        // случайная цифра. null = калибровки не было.
        val cm = StrideModel.measuredStrideCm(this)
        val msg = "Дистанция — это шаги, умноженные на длину твоего шага.\n\n" +
            (if (cm != null) "Твоя измеренная длина шага: " + cm + " см.\n\n"
             else "Длина шага пока не измерена — берётся оценка по росту. Калибровка сделает дистанцию честной.\n\n") +
            "GPS в фоне не работает: он сажает батарею и врёт в лесу и " +
            "между домами. Поэтому дистанция считается из шагов — их чип " +
            "считает точно.\n\n" +
            "Откуда берётся длина шага:\n" +
            "— измеренная тобой на калибровке — самый честный вариант;\n" +
            "— если калибровки нет, берётся оценка по росту, и это " +
            "приблизительно.\n\n" +
    ...
    private fun showActiveTimeInfo() {
        val msg = "Активное время — это время, когда ты реально двигался, " +
            "а не время с телефоном в кармане.\n\n" +
            "Час засчитывается по шагам внутри него: были шаги — время " +
            "идёт, не было — не идёт. Стоянка на светофоре внутри прогулки " +
            "не вырезается: пауза принадлежит прогулке.\n\n" +
            "Что НЕ попадает сюда:\n" +
            "— поездки в транспорте: тряска не движение;\n" +
